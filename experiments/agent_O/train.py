"""
Agent O — Training script (LightGBM, cross-asset features, SOL 4h).

- Loads SOL 4h, BTC 4h, BTC 1d, ETH 4h, ETH 1d, ETH/BTC daily, BTC funding.
- HARD CUTOFF <=2025-12-31 enforced on every source.
- Builds 16 cross-asset features and binary labels (TP=4%/SL=2.5%/12 bars).
- Runs PURGED time-series CV (11+ folds, gap=84 bars=14 days).
- LightGBM with anti-overfitting caps; threshold calibrated on TRAIN.
...
GAP_BARS = 84            # 14 days at 4h (>= label horizon 12 bars)
MIN_TRAIN_BARS = 2000    # ~ 1 year at 4h
N_SPLITS = 12

# Anti-overfitting caps from brief
LGB_PARAMS = dict(
    objective="binary",
    metric="auc",
    learning_rate=0.03,
...
def build_dataset(sol_4h, btc_4h, eth_4h, btc_1d, eth_1d, ethbtc, fund):
    feats = strategy.build_features(
        sol_4h, btc_4h, eth_4h, btc_1d, eth_1d, ethbtc, fund
    )
...
def calibrate_threshold(model, scaler, X_train, y_train,
    ...
    >= min. SOL label balance is lower (TP4/SL2.5 -> ~36%); precision target
    0.50 means model must beat the base rate on its high-prob bucket. NEVER
    touches test data."""
...
def main():
    print("=== Agent O - Train (LightGBM cross-asset, SOL 4h) ===")
    print(f"Cutoff: {CUTOFF}")

    sol_4h, btc_4h, eth_4h, btc_1d, eth_1d, ethbtc, fund = load_data()
    print(f"SOL 4h bars (<= cutoff): {len(sol_4h)}  "
          f"range: {sol_4h.index[0]} -> {sol_4h.index[-1]}")
    print(f"BTC 4h: {len(btc_4h)}  ETH 4h: {len(eth_4h)}  "
          f"BTC 1d: {len(btc_1d)}  ETH 1d: {len(eth_1d)}  "
          f"ETH/BTC: {len(ethbtc)}  funding: {len(fund)}")

    X, y = build_dataset(sol_4h, btc_4h, eth_4h, btc_1d, eth_1d, ethbtc, fund)
    ...
    for k, (tr_idx, te_idx) in enumerate(purged_ts_split(len(X)), 1):
        ...
        sim = evaluate_fold(sol_4h, X, model, scaler, thr, X_te.index)

        if not sim["trades"].empty:
            tr_copy = sim["trades"].copy()
            tr_copy["fold"] = k
            all_wf_trades.append(tr_copy)
    ...
    if all_wf_trades:
        wf_all = pd.concat(all_wf_trades, ignore_index=True)
        wf_all.to_csv(HERE / "wf_trades.csv", index=False)
    print("\nSaved: model.pkl, scaler.pkl, trained_params.json, "
          "cv_results.csv, wf_trades.csv")
