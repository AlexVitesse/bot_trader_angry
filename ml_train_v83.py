"""
ML V8.3: Macro-Aware Trading - Sizing + Risk-Off Rules
=======================================================
V8.2 FALLIDO: gate binario bloqueaba buenos Y malos dias.
V8.3 NUEVO ENFOQUE:

  1. MacroScorer (ML): predice "calidad del macro para crypto"
     -> NO bloquea, ajusta TAMANO de posicion [0.5x a 1.5x]

  2. Macro Risk-Off (reglas simples):
     -> DXY sube + SPY baja = override a BEAR
     -> Flight to safety (oro >> SPY) = override a BEAR
     -> NO es ML, son reglas de trading profesional

  3. V7 Signal Models: SIN CAMBIOS

4 configs comparadas:
  A) V7 solo (baseline)
  B) V7 + MacroSizing (posiciones mas grandes/chicas segun macro)
  C) V7 + RiskOff (override regime cuando macro grita peligro)
  D) V7 + Both (sizing + risk-off)

Ejecutar: python -u ml_train_v83.py
"""
...
# ============================================================
# 2. MACRO SCORER (ML - continuous, not binary)
# ============================================================
...
def train_macro_scorer(X, y, n_trials=30):
    """Train MacroScorer: LightGBM classifier that outputs probability [0,1]."""
...
# ============================================================
# 3. MACRO RISK-OFF RULES (simple, no ML)
# ============================================================
def compute_risk_off_dates(macro_feat):
    """Compute dates where macro says RISK-OFF (override to BEAR).

    Rules (all use shift(1) data from macro_feat):
    1. Dollar strengthening fast + Stocks falling = risk-off
    2. Gold massively outperforming stocks = flight to safety
    3. Treasury yields spiking = liquidity tightening

    Returns: set of dates (strings 'YYYY-MM-DD') where risk-off triggered
    """
    risk_off = set()
    ...
    for dt in macro_feat.index:
        ...
        # Rule 1: DXY up >1% in 5d AND SPY down >1% in 5d
        dxy5 = row.get('dxy_ret_5d', 0)
        spy5 = row.get('spy_ret_5d', 0)
        if dxy5 > 0.01 and spy5 < -0.01:
            risk_off.add(ds)
            continue

        # Rule 2: Gold outperforming SPY by >2.5% in 5d (flight to safety)
        gsr = row.get('gold_spy_ratio', 0)
        if gsr > 0.025:
            risk_off.add(ds)
            continue

        # Rule 3: Dollar surging >2% in 5d (strong risk-off)
        if dxy5 > 0.02:
            risk_off.add(ds)
...
# ============================================================
# 4. BACKTEST WITH MACRO INTELLIGENCE
# ============================================================
def backtest_with_macro(all_data, regime_daily, models,
                         ...
                         use_macro_riskoff=False,
                         thresh=0.7, use_trailing=True, use_atr=True,
                         max_notional=MAX_NOTIONAL):
    """V7 backtest + macro intelligence.

    Two independent improvements:
    1. Macro Sizing: scale position size by macro score [0.5x to 1.5x]
    2. Macro Risk-Off: override regime to BEAR on risk-off dates
    """
    # Timeline
    all_t = set()
    for p in models:
    ...
    for t in timeline:
        ...
        # Regime: V7 base, with optional risk-off override
        reg = regime_4h.loc[t] if t in regime_4h.index else 'RANGE'

        if use_macro_riskoff and ds in risk_off_dates:
            if reg != 'BEAR':
                reg = 'BEAR'
                if t.hour == 0:
                    riskoff_days += 1
        ...
        # Macro sizing multiplier for today
        # score=0.5 (neutral) -> mult=1.0 | score=0 -> mult=0.5 | score=1 -> mult=1.5
        macro_score = macro_scores.get(ds, 0.5)
        sizing_mult = 0.5 + macro_score if use_macro_sizing else 1.0
        ...
        for pair, info in models.items():
            ...
            # Regime filter (with potential risk-off override)
        ...
        for cand in cands:
            ...
            # Sizing: V7 base * macro multiplier
...
def main():
    ...
    # Risk-off dates
    risk_off = compute_risk_off_dates(macro_feat)
    print(f'  Risk-off dates: {len(risk_off)} dias ({len(risk_off)/len(macro_feat)*100:.1f}%)')

    # ================================================================
    # WALK-FORWARD: 4 configs x 4 folds
    # ================================================================
    print('\n' + '=' * 70)
    print('WALK-FORWARD: V7 vs V7+Sizing vs V7+RiskOff vs V7+Both')
    print('=' * 70)
    ...
    CONFIGS = ['V7', 'Sizing', 'RiskOff', 'Both']
    ...
    for tr_e, te_s, te_e, fn in folds:
        ...
        results = {}
        for cfg in CONFIGS:
            use_sz = cfg in ('Sizing', 'Both')
            use_ro = cfg in ('RiskOff', 'Both')
            ...
            if cfg == 'V7':
                ...
                tr, mdd, kil, stats = backtest_with_macro(
                    all_data, regime, mdls_f,
                    macro_scorer=scorer if use_sz else None,
                    macro_feat=macro_feat, macro_fcols=scorer_fcols,
                    risk_off_dates=risk_off if use_ro else None,
                    use_macro_sizing=use_sz,
                    use_macro_riskoff=use_ro,
                )
            results[cfg] = (tr, mdd, kil, stats)

        # Print fold results
        for cfg in CONFIGS:
            tr, mdd, kil, stats = results[cfg]
            if not tr:
                print(f'    {cfg:<10} Sin trades')
                continue
            fin = tr[-1]['bal']
    ...
    for cfg in CONFIGS:
        res = all_results[cfg]
        if not res:
            print(f'  {cfg:<10} Sin resultados'); continue
        ...
        print(f'  {cfg:<10} {prof}/{len(res)} folds+ | '
    ...
    for cfg in CONFIGS:
        ...
        print_results(tr, mdd, kil, f'{cfg}{extra}')
        full_metrics[cfg] = extract_metrics(tr, mdd, kil)
    ...
    best_cfg = None
    best_m = None
    for cfg in ['Both', 'Sizing', 'RiskOff']:
        m = full_metrics.get(cfg)
        if m and v7:
            improves_pf = m['pf'] > v7['pf']
            improves_dd = m['dd'] < v7['dd']
            improves_sharpe = m['sharpe'] > v7['sharpe']
            if (improves_pf and improves_dd) or (improves_pf and improves_sharpe):
                best_cfg = cfg
                best_m = m
                break
    ...
    if best_m and v7:
        ...
        if best_m['pf'] > 1.5 and best_m['dd'] < 15:
            print(f'        APROBADO para produccion')
        else:
            print(f'        Mejora parcial, seguir iterando')
    else:
        print(f'    >>> Ninguna config macro mejora consistentemente vs V7')
        print(f'        Intentar: mas features, diferentes reglas, otro approach')
